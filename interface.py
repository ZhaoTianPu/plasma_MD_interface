...
def interface(sim):
  ...
  for iGrid in range(sim.NGrid):
    ...
    L.fix("Wall"+"_"+str(iGrid), "all", "wall/lj126", "xlo",-sim.Lx2+iGrid*sim.dx, Rkcal*min(sim.Ti,sim.Te), sim.aWSmaxi, sim.aWSmaxi, "xhi", -sim.Lx2+(iGrid+1)*sim.dx, Rkcal*sim.T, sim.aWSmaxi, sim.aWSmaxi, "units", "box", "pbc", "yes")
